...
class SearchAgent:
    ...
    def search(self, query: str, publisher: str | None = None) -> list[dict]:
        ...
        # ── Always surface matching sponsored answers (Main Street OS) ──
        # LLMs do not reliably call search_sponsored_answers even when the
        # system prompt requires it, so we run it once here per user query
        # using the original query text. This fires regardless of which
        # tools the LLM chose (ads / directory / articles). Exactly-once
        # per query means impression counts stay honest.
        try:
            sponsored = self.search_tools.search_sponsored_answers(query=query)
            if sponsored:
                logger.info(
                    f"Post-hoc sponsored search surfaced {len(sponsored)} "
                    f"answer(s) for query: {query!r}"
                )
                all_results.extend(sponsored)
        except Exception as e:
            logger.warning(f"Post-hoc sponsored search failed: {e}")
