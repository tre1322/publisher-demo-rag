...
The DB seeds Westbrook Auto & Tire on first boot. The seed lives in `app/seed.py` and only runs if the `businesses` table is empty — it's safe to restart the server repeatedly.
...
│   ├── seed.py             ← Westbrook Auto & Tire seed data
