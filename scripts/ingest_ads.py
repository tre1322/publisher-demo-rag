#!/usr/bin/env python
"""Ingest advertisements from files into the database."""

import argparse
import csv
import json
import logging
import re
import sys
import uuid
from datetime import datetime, timedelta
from pathlib import Path

from bs4 import BeautifulSoup

# Add parent directory to path for imports
sys.path.insert(0, str(Path(__file__).parent.parent))

from src.modules.advertisements import insert_advertisement, get_advertisement_count
from src.metadata_extractor_ads import AdMetadataExtractor

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

ADS_DIR = Path(__file__).parent.parent / "data" / "ads"


def parse_txt_file(file_path: Path) -> list[dict]:
    """Parse a plain text ad file.

    Args:
        file_path: Path to the text file.

    Returns:
        List containing single ad dict with raw_text.
    """
    raw_text = file_path.read_text(encoding="utf-8")
    return [{"raw_text": raw_text, "source_file": file_path.name}]


def parse_html_file(file_path: Path) -> list[dict]:
    """Parse an HTML ad file, extracting text content.

    Args:
        file_path: Path to the HTML file.

    Returns:
        List containing single ad dict with raw_text.
    """
    html_content = file_path.read_text(encoding="utf-8", errors="ignore")
    soup = BeautifulSoup(html_content, "html.parser")

    # Remove script and style elements
    for element in soup(["script", "style", "head", "meta", "link"]):
        element.decompose()

    # Get text and clean up whitespace
    text = soup.get_text(separator="\n")
    # Clean up excessive whitespace while preserving paragraph breaks
    lines = [line.strip() for line in text.splitlines()]
    text = "\n".join(line for line in lines if line)
    # Collapse multiple newlines to max 2
    text = re.sub(r"\n{3,}", "\n\n", text)

    return [{"raw_text": text.strip(), "source_file": file_path.name}]


def parse_json_file(file_path: Path) -> list[dict]:
    """Parse a JSON ad file.

    Args:
        file_path: Path to the JSON file.

    Returns:
        List of ad dictionaries.
    """
    content = file_path.read_text(encoding="utf-8")
    data = json.loads(content)

    # Handle single object or array
    if isinstance(data, dict):
        data = [data]

    # Add source file and raw_text for each ad
    for ad in data:
        ad["source_file"] = file_path.name
        ad["raw_text"] = json.dumps(ad, indent=2)

    return data


def parse_csv_file(file_path: Path) -> list[dict]:
    """Parse a CSV ad file.

    Args:
        file_path: Path to the CSV file.

    Returns:
        List of ad dictionaries.
    """
    ads = []
    with open(file_path, newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Convert empty strings to None
            ad = {k: (v if v else None) for k, v in row.items()}

            # Convert numeric fields
            for field in ["price", "original_price", "discount_percent"]:
                if ad.get(field):
                    try:
                        ad[field] = float(ad[field])
                    except ValueError:
                        ad[field] = None

            ad["source_file"] = file_path.name
            ad["raw_text"] = json.dumps(row)
            ads.append(ad)

    return ads


def ingest_ad(
    ad_data: dict,
    extractor: AdMetadataExtractor | None = None,
    default_valid_days: int = 30,
    publisher: str | None = None,
) -> bool:
    """Ingest a single advertisement.

    Args:
        ad_data: Advertisement data dictionary.
        extractor: Optional metadata extractor for AI enhancement.
        default_valid_days: Default validity period if not specified.
        publisher: Name of the publishing newspaper.

    Returns:
        True if successful.
    """
    raw_text = ad_data.get("raw_text", "")
    source_file = ad_data.get("source_file", "unknown")

    # Check if this is raw text that needs full extraction
    needs_full_extraction = (
        "advertiser" not in ad_data or "product_name" not in ad_data
    )

    if needs_full_extraction and extractor:
        logger.info(f"Extracting metadata from raw text: {source_file}")
        extracted = extractor.extract_from_raw_text(raw_text)
        # Merge extracted data with any existing fields
        for key, value in extracted.items():
            if key not in ad_data or ad_data[key] is None:
                ad_data[key] = value
    elif extractor and not needs_full_extraction:
        # Enhance existing structured data
        enhanced = extractor.enhance_metadata(
            advertiser=ad_data.get("advertiser", ""),
            product_name=ad_data.get("product_name", ""),
            description=ad_data.get("description"),
            category=ad_data.get("category"),
            price=ad_data.get("price"),
            original_price=ad_data.get("original_price"),
            discount_percent=ad_data.get("discount_percent"),
        )
        # Apply enhancements for missing fields only
        for key, value in enhanced.items():
            if key not in ad_data or ad_data[key] is None:
                ad_data[key] = value

    # Validate required fields
    if not ad_data.get("advertiser") or not ad_data.get("product_name"):
        logger.error(f"Missing required fields in {source_file}")
        return False

    # Set default dates if not provided
    today = datetime.now()
    if not ad_data.get("valid_from"):
        ad_data["valid_from"] = today.strftime("%Y-%m-%d")
    if not ad_data.get("valid_to"):
        ad_data["valid_to"] = (today + timedelta(days=default_valid_days)).strftime(
            "%Y-%m-%d"
        )

    # Generate ID
    ad_id = str(uuid.uuid4())

    # Insert into database
    insert_advertisement(
        ad_id=ad_id,
        product_name=ad_data.get("product_name", ""),
        advertiser=ad_data.get("advertiser", ""),
        description=ad_data.get("description"),
        category=ad_data.get("category"),
        price=ad_data.get("price"),
        original_price=ad_data.get("original_price"),
        discount_percent=ad_data.get("discount_percent"),
        valid_from=ad_data.get("valid_from"),
        valid_to=ad_data.get("valid_to"),
        url=ad_data.get("url"),
        raw_text=raw_text,
        publisher=publisher,
    )

    logger.info(
        f"Ingested ad: {ad_data.get('advertiser')} - {ad_data.get('product_name')}"
    )
    return True


def ingest_file(
    file_path: Path,
    extractor: AdMetadataExtractor | None = None,
    publisher: str | None = None,
) -> dict:
    """Ingest ads from a single file.

    Args:
        file_path: Path to the file.
        extractor: Optional metadata extractor.
        publisher: Name of the publishing newspaper.

    Returns:
        Results summary dict.
    """
    results = {"file": file_path.name, "success": 0, "failed": 0, "errors": []}

    suffix = file_path.suffix.lower()

    try:
        if suffix == ".txt":
            ads = parse_txt_file(file_path)
        elif suffix == ".json":
            ads = parse_json_file(file_path)
        elif suffix == ".csv":
            ads = parse_csv_file(file_path)
        elif suffix in (".html", ".htm"):
            ads = parse_html_file(file_path)
        else:
            results["errors"].append(f"Unsupported file type: {suffix}")
            return results
    except Exception as e:
        results["errors"].append(f"Failed to parse file: {e}")
        return results

    for ad in ads:
        try:
            if ingest_ad(ad, extractor, publisher=publisher):
                results["success"] += 1
            else:
                results["failed"] += 1
        except Exception as e:
            results["failed"] += 1
            results["errors"].append(str(e))

    return results


def ingest_all(
    directory: Path = ADS_DIR,
    use_metadata_extraction: bool = True,
    publisher: str | None = None,
) -> dict:
    """Ingest all ad files from a directory.

    Args:
        directory: Directory containing ad files.
        use_metadata_extraction: Whether to use AI metadata extraction.
        publisher: Name of the publishing newspaper.

    Returns:
        Summary of ingestion results.
    """
    results = {
        "total_files": 0,
        "total_ads": 0,
        "failed_ads": 0,
        "errors": [],
    }

    if not directory.exists():
        logger.warning(f"Directory does not exist: {directory}")
        return results

    # Initialize extractor if needed
    extractor = None
    if use_metadata_extraction:
        try:
            extractor = AdMetadataExtractor()
            logger.info("AI metadata extraction enabled")
        except Exception as e:
            logger.warning(f"Could not initialize metadata extractor: {e}")

    # Find all supported files
    files = (
        list(directory.glob("*.txt"))
        + list(directory.glob("*.json"))
        + list(directory.glob("*.csv"))
        + list(directory.glob("*.html"))
        + list(directory.glob("*.htm"))
    )
    results["total_files"] = len(files)

    for file_path in files:
        logger.info(f"Processing: {file_path.name}")
        file_results = ingest_file(file_path, extractor, publisher=publisher)
        results["total_ads"] += file_results["success"]
        results["failed_ads"] += file_results["failed"]
        results["errors"].extend(file_results["errors"])

    return results


def main() -> None:
    """Run ad ingestion from command line."""
    parser = argparse.ArgumentParser(
        description="Ingest advertisements from files into the database"
    )
    parser.add_argument(
        "--file",
        "-f",
        type=str,
        help="Ingest a specific file instead of all files in data/ads/",
    )
    parser.add_argument(
        "--no-metadata",
        action="store_true",
        help="Disable AI metadata extraction",
    )
    parser.add_argument(
        "--dir",
        "-d",
        type=str,
        default=str(ADS_DIR),
        help="Directory to ingest from (default: data/ads/)",
    )
    parser.add_argument(
        "--publisher",
        "-p",
        type=str,
        help="Name of the publishing newspaper",
    )

    args = parser.parse_args()

    print("=" * 60)
    print("Publisher RAG Demo - Advertisement Ingestion")
    print("=" * 60)

    if args.file:
        file_path = Path(args.file)
        if not file_path.exists():
            print(f"Error: File not found: {file_path}")
            sys.exit(1)

        extractor = None
        if not args.no_metadata:
            try:
                extractor = AdMetadataExtractor()
            except Exception as e:
                print(f"Warning: Could not initialize metadata extractor: {e}")

        results = ingest_file(file_path, extractor, publisher=args.publisher)
        print(f"\nFile: {results['file']}")
        print(f"Ads ingested: {results['success']}")
        print(f"Ads failed: {results['failed']}")
    else:
        directory = Path(args.dir)
        results = ingest_all(
            directory=directory,
            use_metadata_extraction=not args.no_metadata,
            publisher=args.publisher,
        )

        print(f"\nFiles processed: {results['total_files']}")
        print(f"Ads ingested: {results['total_ads']}")
        print(f"Ads failed: {results['failed_ads']}")

    if results.get("errors"):
        print("\nErrors:")
        for error in results["errors"]:
            print(f"  - {error}")

    print(f"\nTotal ads in database: {get_advertisement_count()}")


if __name__ == "__main__":
    main()
